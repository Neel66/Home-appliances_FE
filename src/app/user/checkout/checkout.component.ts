...
export class CheckoutComponent implements OnInit {
  ...
  getaddress() {
    this.service.getaddress(this.userid).subscribe((res) => {
      if (res.status == 1) {
  ...
  getpayment() {
    this.service.getpayment(this.userid).subscribe((res) => {
      console.log(res);
      this.payment = res.data[0]?.total;
      this.total = this.payment;
    });
  }
  onSelect(id: number) {
